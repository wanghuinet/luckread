...
6. Repository inspection confirms `src/collections/Users.ts` enables Payload auth and `src/payload.config.ts` uses `sqliteD1Adapter`, `push: false`, and `src/migrations`; the current main tree has no executable migration artifact that can establish actual D1 schema evidence.

...
5. `contracts/entity/entity-catalog.v1.json` establishes `ENT-USER` as the currently VERIFIED authoritative User entity and points to the legacy root `workers/W01-payload/src/collections/Users.ts`; this is not the active W01 runtime authority.
...
8. The legacy root `workers/W01-payload/src/collections/Users.ts` is concrete historical scaffold evidence for the six previously documented fields, but it is not the active W01 implementation. Current W01 `workers/W01-payload/src/collections/Users.ts` has `slug: users`, `auth: true`, and `fields: []`.

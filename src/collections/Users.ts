  ...
  endpoints: [
    {
      ...
      handler: async (req) => {
        ...
        const grants = await req.payload.find({
          collection: 'entitlement-grants',
          where: { and: [{ user: { equals: String(req.user.id) } }, { status: { equals: 'ACTIVE' } }] },
          depth: 1,
          limit: 100,
          overrideAccess: true,
          req,
        })
        return Response.json({ data: grants.docs })
      },
      custom: { openapi: { summary: 'List current user entitlements' } },
    },
    {
      ...
      handler: async (req) => {
        ...
        return Response.json({ data: memberships.docs })
      },
      custom: { openapi: { summary: 'List current user organization memberships' } },
    },
    {
      path: '/:id/state',
      method: 'post',
      handler: async (req) => {
        ...
        if (Number(current.accountStateVersion) !== body.expectedStateVersion) throw new APIError('Account state version conflict; reload and retry', 409)
        if (transition.precondition === 'phone_or_email_verified' && !current.emailVerifiedAt && !current.phoneVerifiedAt) throw new APIError('Phone or email verification is required', 409)
        if (transition.precondition === 'restore_window') assertAccountRestoreWindow(current.deletedAt)
        if (transition.precondition === 'cooling_off_started' && !current.deletionRequestedAt) throw new APIError('Deletion request timestamp is required before entering the cooling-off period', 409)
        ...
        const result = await req.payload.update({
          collection: 'users',
          where: { and: [{ id: { equals: id } }, { accountState: { equals: from } }, { accountStateVersion: { equals: body.expectedStateVersion } }] },
          data,
          overrideAccess: true,
          req,
        })
        ...
        const event = await req.payload.create({
          ...
          data: {
            eventId: randomUUID(), user: id, fromState: from, toState: body.to,
            actorId: String(user?.id), actorType: transition.actor,
            permission: transition.permission ?? 'self-service', version: nextVersion,

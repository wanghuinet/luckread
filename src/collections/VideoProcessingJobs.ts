...
import { applyVideoTranscodeResult, readTranscodeManifest } from '../lib/video-transcode-service'
  ...
  endpoints: [
    ...
    },
    {
      ...
      handler: async (req) => {
        ...
        if (!id) throw new APIError('Job id is required', 400)
        const job = await req.payload.findByID({ collection: 'video-processing-jobs', id, depth: 0, overrideAccess: true, req })
        if (job.stage !== 'TRANSCODE') throw new APIError('Only TRANSCODE jobs can be run by this endpoint', 409)
        if (job.status !== 'QUEUED') throw new APIError(`Job is already ${job.status}`, 409)
        await req.payload.update({ collection: 'video-processing-jobs', id, data: { status: 'RUNNING', attempts: Number(job.attempts ?? 0) + 1, startedAt: new Date().toISOString(), lastErrorCode: null, lastErrorMessage: null }, overrideAccess: true, req })
        try {
          const assetId = typeof job.videoAsset === 'object' && job.videoAsset !== null ? String(job.videoAsset.id) : String(job.videoAsset)
          const asset = await req.payload.findByID({ collection: 'video-assets', id: assetId, depth: 0, overrideAccess: true, req })
          const variants = await readTranscodeManifest(String(asset.sourceR2Key))
          await applyVideoTranscodeResult(req, asset.id, variants)
          await req.payload.update({ collection: 'video-processing-jobs', id, data: { status: 'SUCCEEDED', finishedAt: new Date().toISOString() }, overrideAccess: true, req })
          await req.payload.create({ collection: 'video-processing-jobs', data: { jobId: randomUUID(), videoAsset: asset.id, stage: 'POSTER', status: 'QUEUED', attempts: 0, maxAttempts: 3 }, overrideAccess: true, req })
          return Response.json({ data: { status: 'SUCCEEDED', nextStage: 'POSTER', variantCount: variants.length } })
        } catch (error) {
          const message = error instanceof Error ? error.message : 'Video transcode failed'
          const assetId = typeof job.videoAsset === 'object' && job.videoAsset !== null ? String(job.videoAsset.id) : String(job.videoAsset)
          await req.payload.update({ collection: 'video-processing-jobs', id, data: { status: 'FAILED', finishedAt: new Date().toISOString(), lastErrorCode: 'TRANSCODE_FAILED', lastErrorMessage: message }, overrideAccess: true, req })
          await req.payload.update({ collection: 'video-assets', id: assetId, data: { state: 'FAILED', failureCode: 'TRANSCODE_FAILED', failureMessage: message, lastAttemptAt: new Date().toISOString() }, overrideAccess: true, req })
          throw error
        }
      },
    },

...
export const VideoProcessingJobs: CollectionConfig = {
  ...
  endpoints: [{
    ...
    handler: async (req) => {
      ...
      const running = await req.payload.update({ collection: 'video-processing-jobs', id, data: { status: 'RUNNING', attempts: Number(job.attempts ?? 0) + 1, startedAt: new Date().toISOString(), lastErrorCode: null, lastErrorMessage: null }, overrideAccess: true, req })
      try {
        const asset = await req.payload.findByID({ collection: 'video-assets', id: job.videoAsset, depth: 0, overrideAccess: true, req })
        ...
        await req.payload.update({ collection: 'video-processing-jobs', id: running.id, data: { status: 'FAILED', finishedAt: new Date().toISOString(), lastErrorCode: 'PROBE_FAILED', lastErrorMessage: message }, overrideAccess: true, req })
        await req.payload.update({ collection: 'video-assets', id: job.videoAsset, data: { state: 'FAILED', failureCode: 'PROBE_FAILED', failureMessage: message, lastAttemptAt: new Date().toISOString() }, overrideAccess: true, req })
        throw error
      }
    },
  }],

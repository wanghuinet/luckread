  ...
  endpoints: [
    {
      ...
      handler: async (req) => {
        ...
        assertContentTransitionActor({ transition, user: req.user as { id: string | number; role?: string }, authorId })
      ...
      custom: {
        openapi: {
          ...
          requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', required: ['to', 'expectedVersion', 'expectedRevision'], properties: { to: { type: 'string', enum: contentStates }, expectedVersion: { type: 'integer', minimum: 1 }, expectedRevision: { type: 'integer', minimum: 1 } } } } } },
          responses: { 200: { description: 'Content state transitioned' }, 400: { description: 'Invalid transition request' }, 401: { description: 'Authentication required' }, 403: { description: 'Permission denied' }, 409: { description: 'Invalid transition or optimistic concurrency conflict' } },
        },
      },
    },
  ],

...
- Decision: GAP RECORDED; current Mapping status remains fail-closed; no business implementation change is authorized by this record.
...
5. The current Payload native inventory generator, `scripts/payload-native-inventory.mjs`, still reads root `src/payload.config.ts` and root `src/collections/`.
6. Therefore the committed Payload inventory and related derived evidence currently describe the root Payload scaffold, not the declared W01 runtime authority.
7. The current canonical Entity catalog points `ENT-USER` implementation evidence at the root `src/collections/Users.ts`; this is insufficient to claim current W01 field implementation.
